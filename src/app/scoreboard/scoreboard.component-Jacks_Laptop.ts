...
loadEPLStatsData() {
  const EPLStatsUrl = 'assets/EPL-Stats.csv';
  this.http.get(EPLStatsUrl, { responseType: 'text' })
    .toPromise()
    .then(csvData => {
      this.eplStats = this.parseEPLStats(csvData);
    })
    .catch(error => {
      console.error('Error loading EPL stats data:', error);
    });
}


parseEPLStats(csvData: string | undefined) {
  const xGoals: { [team: string]: { GF: number; GA: number; wGF: number, wGA: number,
    HomeAdv: number } } = {};
...
parseNHLStats(csvData: string | undefined) {
  ...
  async runSimulation() {
   ...
   if (homeTeamNFL && awayTeamNFL) {
    ...
    const HrYdsAtt = (.85 * homeTeamNFL.RushYdsAtt * (awayTeamNFL.dRushYdsAtt / 4.))+(.15* 4.03); //4.45 = avg
    const ArYdsAtt = (.85 * awayTeamNFL.RushYdsAtt * (homeTeamNFL.dRushYdsAtt / 4.03)) + (.15*4.03); //4.45 = avg
    ...
    const HpYdsAtt = (.85 * homeTeamNFL.PassYdsAtt * (awayTeamNFL.dPassYdsAtt / 6.79)) + (.15* 6.79); //6.55 = avg
    const ApYdsAtt = (.85 * awayTeamNFL.PassYdsAtt * (homeTeamNFL.dPassYdsAtt / 6.79)) + (.15*6.79); //6.55 = avg
    const HpAtt = (.85 * homeTeamNFL.oPassPerGame * (awayTeamNFL.dPassPerGame / 34.41)) + (.15 * 34.41); //33.42 avg
    const ApAtt = (.85 * awayTeamNFL.oPassPerGame * (homeTeamNFL.dPassPerGame / 34.41)) + (.15* 34.41); //33.42 avg
    ...
    const HoYP = (.85 * homeTeamNFL.oYdsPerPoint * (awayTeamNFL.dYdsPerPoint / 15)) + (.15* 15); //15.55 = avg
    const AoYP = (.85 * awayTeamNFL.oYdsPerPoint * (homeTeamNFL.dYdsPerPoint / 15)) + (.15 * 15); //15.55 = avg
    const hYdsScore = hTotalYds / HoYP;
    const AYdsScore = aTotalYds / AoYP;
    const HoPPlay = (.85 * homeTeamNFL.oPtsPerPlay * (awayTeamNFL.dPtsPerPlay / 0.357)) + (.15 * .357); // .3493 avg
    const AoPPlay = (.85 * awayTeamNFL.oPtsPerPlay * (homeTeamNFL.dPtsPerPlay / 0.357)) + (.15 * .357); // .3493 avg
    const hPlaysGame = (.85 * homeTeamNFL.oPlaysGame * (awayTeamNFL.dPlaysGame / 63.84)) + (.15* 63.84); //63.04 avg
    const aPlaysGame = (.85 * awayTeamNFL.oPlaysGame * (homeTeamNFL.dPlaysGame / 63.84)) + (.15* 63.84); //63.04 avg
...
console.log(awaySingle[0],awayDouble[0],awayTriple[0],awayHR[0],awayK[0], awayBB[0],awayXbaValues[0],awayXobp[0],awayXslg[0])
  ...
  break;
      ...
      case 'EPL':
            const homeTeamEPL = this.eplStats[this.selectedHomeTeam];
            const awayTeamEPL = this.eplStats[this.selectedAwayTeam];
            if (homeTeamEPL && awayTeamEPL) {
    
              const homeAdvModifier = this.isNeutralSite ? 0 : homeTeamEPL.HomeAdv;
          
              // Calculate the home team's total score
              this.homeTeamScore = Number(((homeTeamEPL.GF * awayTeamEPL.wGA) * (1 + homeAdvModifier)).toFixed(3));
          
              // Calculate the away team's total score
              this.awayTeamScore = Number(((awayTeamEPL.GF * homeTeamEPL.wGA) * (1 - homeAdvModifier) ).toFixed(3));
            }
            this.showDetailsButton = true;
